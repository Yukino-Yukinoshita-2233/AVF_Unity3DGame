     ...
     , eventData.enterEventCamera, out outPos))
        {
            ...
            foreach (var h in hits2d)
            {
                if (h.collider.transform.gameObject.layer == 9)
                {
                    ...
                    if (PuzzleManager.Instance.IstherePiece(gridID))
                    {
                        PuzzleManager.Instance.OutPiece(gridID);
                        Debug.Log("拿出碎片");
                    }
                }
            }
            ...
            foreach (var h in hits2d)
            {
                if (h.collider.transform.gameObject.layer == 9)
                {
                    ...
                    if (!PuzzleManager.Instance.IstherePiece(gridID))
                    {
                        ...
                        PuzzleManager.Instance.SetPiece(gridID, int.Parse(rectTrans.name));
                        rectTrans.SetAsFirstSibling();
                        if (PuzzleManager.Instance.IsFinish())
                        {
                            Debug.Log("完成");
                        }
                    }
