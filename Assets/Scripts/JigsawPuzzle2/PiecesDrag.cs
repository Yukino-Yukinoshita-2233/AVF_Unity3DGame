...
public class PiecesDrag2 : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    //遮罩的图片
    Image maskImage;
    //鼠标点击偏移
     ...
     , eventData.enterEventCamera, out outPos))
        {
            ...
            foreach (var h in hits2d)
            {
                if (h.collider.transform.gameObject.layer == 9)
                {
                    ...
                    if (PuzzleManager2.Instance.CheckHavePiece(gridID))
                    {
                        PuzzleManager2.Instance.OutPiece(gridID);
                        Debug.Log("拿出碎片");
                    }
                }
            }
            ...
            foreach (var h in hits2d)
            {
                if (h.collider.transform.gameObject.layer == 9)
                {
                    ...
                    if (!PuzzleManager2.Instance.CheckHavePiece(gridID))
                    {
                        ...
                        PuzzleManager2.Instance.SetPiece(gridID, int.Parse(rectTrans.name));
                        rectTrans.SetAsFirstSibling();
                        if (PuzzleManager2.Instance.IsFinish())
                        {
                            Debug.Log("完成");
                        }
                    }
